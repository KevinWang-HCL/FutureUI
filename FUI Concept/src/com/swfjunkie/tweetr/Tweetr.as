package com.swfjunkie.tweetr
{
    ...
    public class Tweetr extends EventDispatcher
    {
		...
		private static const URL_UPLOAD_IMAGE:String =              "/statuses/update_with_media.xml"; 
		...
		private static const URL_TWITTER_IMAGE_UPLOAD_API:String =   "http://upload.twitter.com/1";
		private static const URL_TWITTER_API:String =                "https://api.twitter.com/1";
		...
		private var apiServer:String = URL_TWITTER_API;
        //--------------------------------------------------------------------------
        //
        //  Properties
        //
        ...
        private function get url():URLRequest
        {
            if (_oAuth)
            {
                ...
                var signedData:String = _oAuth.getSignedRequest(urlRequest.method, _serviceHost+request, urlRequest.data as URLVariables);
        ...
        private var _serviceHost:String = URL_TWITTER_API;
        /**
		...
		protected function ioErrorFun(event:IOErrorEvent):void
		{
		...
		public function uploadImage(status:String, imagePath:String, inReplyTo:Number = 0, 
									lat:Number = 0, long:Number = 0, possibly_sensitive:Boolean = false,
									display_coordinates:Boolean = false):void
		{
			var imageFile:File = new File(imagePath);
			var fileReference:FileReference = imageFile;
			var vars:URLVariables = new URLVariables("status="+status+"&media[]="+fileReference.name);
			
			checkCredentials();
			/*vars.status = strEscape(status.substr(0,120));
			_returnType = RETURN_TYPE_STATUS;
			if (inReplyTo != 0)
				vars.in_reply_to_status_id = inReplyTo;
			if (lat != 0)
			...
			if (possibly_sensitive)
			    vars.possibly_sensitive = true;
			if (display_coordinates)
			    vars.display_coordinates = true;*/
			
			serviceHost = URL_TWITTER_IMAGE_UPLOAD_API;
			setPOSTRequest(vars);
			request = URL_UPLOAD_IMAGE;
			imageFile.addEventListener(IOErrorEvent.IO_ERROR, ioErrorFun);
			
			imageFile.addEventListener(DataEvent.UPLOAD_COMPLETE_DATA, uploadCompleteHandler);
			imageFile.addEventListener(ProgressEvent.PROGRESS, onProgress);
			fileReference.upload(url);
			serviceHost = URL_TWITTER_API;
		}
        ...
        {
			trace("handleFileReferenceUploadComplete");
